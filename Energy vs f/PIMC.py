...
class Path(object):
    ...
    def __init__(self,M,T,delta,m,mu,fs,char,thermalize,lam,n_steps,x_max,n_bins): #Initialize
    ...
    def V(self,x):
        ...
        return 0.5*(self.mu**2)*(x**2) + self.lam*((x**2-self.fs)**2) # Potential Energy(PE)
                                                                        # lam = 0 -> Harmonic Oscillator
                                                                        # lam > 0 -> Anharmonic Oscillator

    def DV(self,x):

        return (self.mu**2)*x + 4*self.lam*x*(x**2-self.fs) # First derivative of Potential Energy
